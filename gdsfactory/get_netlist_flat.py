...
def get_netlist_flat(
    ...
    **kwargs,
    ...
    unique_connections = _simplify_connections(connections)
...
def _simplify_connections(connections):
    """Returns minimal set of nodename: connections"""
    connections_sets = [set(values + [key]) for key, values in connections.items()]
    counter = list(Counter(map(tuple, connections_sets)).keys())
    # Dictionari-fy unique nodes into connections
    return {nodes[0]: nodes[1:] for nodes in counter}
